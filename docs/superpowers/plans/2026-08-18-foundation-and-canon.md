...
14. **One renal observable.** The starter registry declares `egfr` and not
    `crcl`. §5.2 makes them distinct observables and §10.4 gate 15 depends on the
    distinction, but both arrive with the renal content that first needs them
    (§14 step 6). Lab-reported and Noor-derived values are already distinguishable
    at capture by `entry_mode` (`interfaced` vs `noor_derived`, §5), canon
    overwrites nothing, and §5.2's "never silently recomputed under a different
    equation" guarantee is the append-only store's. Equation provenance
    (`reported_equation`, the CKD-EPI 2021 default) is content-plan work, not
    canon's.
...
.github/CODEOWNERS                      # content/ requires the clinical owner's review (§7.5)
...
§7.5: "the pull request *is* the four-eyes approval". A PR only approves clinical content if the right eyes are required on it. Replace both placeholders with real GitHub handles before committing — an unresolvable handle silently disables the rule.

```
# Clinical content requires the named clinical content owner's review (SSOT §7.5).
# The registry's envelopes and conversions are clinical content (§6.4, §6.6).
/content/                @ph1losophrr @CLINICAL-OWNER-HANDLE
/docs/cds-architecture.md @ph1losophrr @CLINICAL-OWNER-HANDLE
```

- [ ] **Step 6: Enable branch protection on `main`**

§14 step 1 names git-as-governance "a prerequisite, not housekeeping". Without protection, `main` accepts a direct push and the PR gate is decorative. Requires admin rights on the repo; run once.

```bash
gh api -X PUT repos/ph1losophrr/Project-Noor/branches/main/protection \
...
Verify: `gh api repos/ph1losophrr/Project-Noor/branches/main/protection --jq '.required_pull_request_reviews.require_code_owner_reviews'`
Expected: `true`.
...
observables:
  ...
  - observable: egfr
    owner: project-owner
    canonical_ucum: "mL/min/1.73m2"
    accepted_units: ["mL/min/1.73m2"]
    conversions: []
    code_unit_map: {}
...
- **§5.2's `crcl` and equation provenance.** This plan registers `egfr` as a
  ...
  `entry_mode` (assumption 14). It does not build: `crcl`, the equation
  identifier and version that a derived value must carry, or the §12.6 gate-15
  claim that a derived value is never recomputed under a different equation than
  the one it was stored with. All three arrive with the derived-value work in
  §14 step 6, alongside the append-only store that makes "never recomputed"
