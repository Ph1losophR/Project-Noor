...
Expected: the 3 template-only tests pass vacuously with no templates on disk while every stylesheet-touching test FAILs with `FileNotFoundError` on `noor.css`.
...
Expected: a collection error — `ImportError: cannot import name 'seed'`, same root cause as written. No test
...
Expected: a collection error — `ImportError: cannot import name 'views'`, same root cause as written.
...
Expected: 9 setup errors (the `day` fixture is not yet added) + 1 404-vs-400 failure, same root cause as written — `/visits` is not a route yet, so
...
Expected: five rows, Abdullah Al-Qahtani first (the store orders by name), Fatima Al-Harbi's row reading
...
Expected: `-k visit_day` matches zero tests by name (run by node id); by node id, 2 failures, both `AttributeError: module 'noor.store' has no attribute
'visit_day'`.
...
Run: `pytest tests/web/test_views.py -v --no-cov`
Expected: the 13 from Task 5 pass; the 16 new ones fail with `AttributeError: module
...
Expected: Abdullah Al-Qahtani first (the store orders by name); Fatima Al-Harbi's row opens on a Scheduled page reading **Routine Visit**, with her
...
`Write-Backs pending: 0.` with `The EMR accepted this Visit's Write-Back.`, because seeded history is marked written-back.
...
Expected: 10 failures. Nine post to routes that do not exist and get 404 from Starlette, same root cause as written;
...
since. Mohammed Al-Shammari's demo Brief shows no `.note` and 7 overdue because his unreachable read is prescribed, not surveillance — the blind-spot rendering itself is proven by test_an_unreadable_item_is_a_blind_spot_and_not_an_overdue_one. Press the one link up and you
