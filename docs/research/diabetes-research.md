...
| Gliclazide 30 mg MR/day | `30 mg, 60 mg, 80 mg` | `strength_achievable` **if** the 30 mg entry is the MR form. The SEML does not state release profile, and MR and IR gliclazide are not interchangeable mg-for-mg — so an ingredient-level rule cannot safely emit an MR dose. This needs `drug_scope_level: product` or `curated_set` (SSOT §7.1e), not `ingredient`. |
...
| Insulin lispro, any unit dose | `200 IU/ml` listed; a `100 IU/ml` presentation may also be listed — **ambiguous** | Same units-only rule applies. The orphan `Suspension for injection: 100 IU/ml` row at §15.1.1 sits between aspart and lispro with its drug name lost in conversion, so lispro's full presentation set is unresolved — **do not author "200 IU/ml is the only concentration"** on this source. What *is* certain is that a U-200 lispro exists in the supply chain, which is sufficient to require the units-only rule. ⛔ re-check the source PDF |
