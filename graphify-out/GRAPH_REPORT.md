...
- [[_COMMUNITY_make_canonical|make_canonical]]
...
- [[_COMMUNITY_Task 12 Report — Fuzzing the boundary|Task 12 Report — Fuzzing the boundary]]
- [[_COMMUNITY_Task 11 Report — Quality resolution repeat confirmation and clinician verification|Task 11 Report — Quality resolution: repeat confirmation and clinician verification]]
- [[_COMMUNITY_Findings-fix report — `featfoundation-and-canon`|Findings-fix report — `feat/foundation-and-canon`]]
...
- [[_COMMUNITY_DeltaVerdict|DeltaVerdict]]
- [[_COMMUNITY_test_snapshot.py|test_snapshot.py]]
- [[_COMMUNITY_MappingInfo|MappingInfo]]
- [[_COMMUNITY_GoalOfCare|GoalOfCare]]
- [[_COMMUNITY__nested_payload|_nested_payload]]
- [[_COMMUNITY_DeltaVerdict|DeltaVerdict]]
- [[_COMMUNITY_Informant|Informant]]
- [[_COMMUNITY_Rule|Rule]]
- [[_COMMUNITY_CanonicalObservation|CanonicalObservation]]
...
Cohesion: 0.14
Nodes (13): 0.1 Document map, 0.2 Table of contents, 0. How to use this document, 13.1 Blocks code (2), 13.2 Blocks patient use, not code (14), 13.3 Explicitly unresolved, 13. Gates, 14. Build sequence (+5 more)
...
Cohesion: 0.25
Nodes (7): Coverage of the brief's test checklist, Files changed, Full gate (all four, in CI order), Self-review findings, Task 2 Report — Snapshot and requested-action contracts, TDD evidence, What was implemented

### Community 28 - "test_import_direction.py"
Cohesion: 0.23
Nodes (15): _called_attributes(), _called_builtin_functions(), _identifiers(), _imported_modules(), Path, _python_files(), The import-direction and purity half of the device-boundary seam (SSOT §4.2)., Every name the code *uses* — not comments or docstrings, which may say     "thr (+7 more)
...
Cohesion: 0.25
Nodes (8): 8.1 The call, 8.2 Every rule considered writes a record, 8.3 The degradation invariant, 8.4 Engine invariants, 8.5 Engine failure semantics, 8. Evaluation, Conflicting patient goals, Requirement verdict vocabulary
...
Cohesion: 0.12
Nodes (26): CanonicalQuantity, ConversionApplied, The conversion that produced a canonical value (SSOT §6.3: "every     conversio, Derived, and it shows its work (§5, §6.3).      `conversion_applied` is None e, from_canonical(), Decimal, Unit resolution and registry-declared conversion (SSOT §6.3).  Resolution is b, No registry declaration covers the requested unit, or a value's recorded     co (+18 more)

### Community 49 - "Task 4 Implementation Report"
Cohesion: 0.25
Nodes (7): Files changed, Gate results, Self-review notes / concerns, Task 4 Report — Threshold, profile, release, and evaluation context, TDD evidence, Test coverage of the brief's Step 1 list, What was implemented

### Community 50 - "Task 5 Report: Unit Resolution"
Cohesion: 0.13
Nodes (14): Changes, Commands and results, Cross-field validators (all `model_validator(mode="after")`), Files, Final-review fix, Findings addressed, Public surface (exactly the brief + resolved decisions, nothing more), Report (+6 more)

### Community 51 - "load_registry"
Cohesion: 0.13
Nodes (18): MappingNode, _ContentLoader, load_registry(), Any, Path, Loads registry content into validated models (SSOT §7.4).  Schema-only YAML (§, The `SafeLoader` subclass §7.5 allows, refusing a repeated mapping key.      S, Load and validate an observable registry file (content/observables/registry.yaml (+10 more)

### Community 53 - "Task 6 Report: Conversion Round-Trip Property"
Cohesion: 0.17
Nodes (11): Changes, Commands & results, Consolidation (sanctioned), Correction, Files changed (commit 6f78e65), Fix wave 1, Implementation summary, Self-review notes (+3 more)
...
Cohesion: 0.22
Nodes (4): DeltaPolicy, Self, Like-with-like comparison rules (§6.1 layer 3)., test_a_delta_policy_must_not_name_unknown_context_fields()
...
Cohesion: 0.14
Nodes (13): Commit 1 — `665b7f3` `test(canon): cover the repeat setting-mismatch guard (100% branch gate)`, Commit 2 — `438d704` `test(canon): boundary fuzz — nothing crosses uncanonicalised; determinism (SSOT §3.1, §6)`, Concerns, Files changed, FIX REPORT — two blockers cleared (controller-approved), `.hypothesis/` state, Issues and concerns, Self-review (+5 more)

### Community 74 - "Task 11 Report — Quality resolution: repeat confirmation and clinician verification"
Cohesion: 0.18
...
Cohesion: 0.25
Nodes (7): Files changed, Issues or concerns, Self-review findings, Task 10 Report — The `canonicalise` pipeline, TDD Evidence, Verification results, What I implemented

### Community 77 - "4. Module architecture"
Cohesion: 0.40
Nodes (5): 4.1 Layout, 4.2 The enforced seam, 4.3.1 Expression shape, 4.3 Why an evaluator and not an inference engine, 4. Module architecture

### Community 78 - "task-12-brief.md"
Cohesion: 0.50
